...
class SaltyBot:
    ...
    def wr_retrieve(self):
        ...
        if game_records == False:
            ...
            sr_game = list(dict(game_records).keys())[0]
            if sr_game.lower() == self.game.lower():
                sr_game_cats = list(game_records[sr_game].keys())
            ...
            if len(categories_in_title) > 1:
                ...
                min_value = min(category_position.values())
        ...
        for i in data_races:
            for races in i['entrants']:
                if self.srl_nick in races:
                    race_channel = i
                    for values in list(race_channel['entrants'].values()):
    ...
    def youtube_video_check(self, vid_type, message):
        ...
        if vid_type == 'short':
            ...
            url_values = urllib.parse.parse_qs(urllib.parse.urlparse(message).query)
            try:
    ...
    def vote(self, message, sender):
        ...
        if self.votes['type'] == 'strict':
            if sender_bet not in self.votes['options']:
                self.twitch_send_message('You must vote for one of the options specified: ' + ', '.join(list(self.votes['options'].keys())), '!vote')
                return
    ...
    def lol_runes(self):
        ...
        for rune in active_page["slots"]:
            ...
            if rune_id not in runes_counted:
                runes_counted[rune_id] = {'id' : rune_id, 'count' : 1, 'description' : runes_list['data'][rune_id]['description'], 'stats' : list(runes_list['data'][rune_id]['stats'].keys())}
            else:
        ...
        for k, v in list(runes_counted.items()):
            for k2, v2 in list(runes_counted.items()):
                if k == k2:
                    continue
        ...
        try:
            ...
            if register: 
                ...
                if register[TYPE] == CHECK:
                    for bot_name,bot_inst in list(bot_dict.items()):
...
def main():
    ...
    for channel_name,channel_data in list(channels_dict.items()): #Start bots
    ...
    while True:
        ...
        if command.startswith("/brcs"):
            for bot,inst in list(bot_dict.items()):
